...
exports.createCourse = async (req, res) => {
    try {
        ...
        const thumbnailLink = await uploadToCloudinary(thumbnail.tempFilePath, process.env.CLOUD_FOLDER)
        //create course
...
exports.updateCourse = async (req, res) => {
    try {
        const { courseName, courseDescription, tag, price, whatYouWillLearn, category: categoryId, courseId } = req.body;
        ...
        if (categoryId) {
            ...
            presentCategory.courses = presentCategory.courses.filter(id => id.toString() !== courseId) || presentCategory.courses
        ...
        if (thumbnailImage) {
            const imageLink = await uploadToCloudinary(thumbnailImage.tempFilePath, process.env.CLOUD_FOLDER)
...
exports.deleteCourse = async (req, res) => {
    try {
        ...
        if (instructor.courses) {
            instructor.courses = instructor.courses.filter(id => id.toString() !== courseId) || instructor.courses;
        ...
        if (studentEnrolled) {
            for (let userId of studentEnrolled) {
                const user = await User.findById(userId)
                user.courses = user.courses ? user.courses.filter(id => id.toString() !== courseId) : user.courses
                await user.save()
            }
        }
