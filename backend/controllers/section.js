...
exports.deleteSection = async (req, res) => { 
    try { 
        ...
        courseDetails.sections = courseDetails.sections.filter(id => id.toString() !== sectionId)
        const updatedCourse = await courseDetails.save()
        return res.status(200).json( { 
            success : true, 
...
exports.addSubSection = async (req ,res) => { 
    try {   
        ...
        //create subsection 
        const videoFile = await uploadToCloudinary(video.tempFilePath , process.env.CLOUD_FOLDER)
...
exports.updateSubSection = async (req ,res)=> { 
    try { 
        ...
        if(video) { 
            const videoFileLink = await uploadToCloudinary(video.tempFilePath , process.env.CLOUD_FOLDER)
...
exports.deleteSubSection = async(req ,res ) => { 
    try { 
        ...
        sectionDetails.subSection  = sectionDetails.subSection.filter(id => id.toString() !== subSectionId)
...
exports.showAllSections = async(req ,res )=> { 
    try { 
        const sectionDetails = await Section.find({ } , { name  : true,}).populate('subSection').exec() 
        return res.status(200).json({ 
            success : true, 
