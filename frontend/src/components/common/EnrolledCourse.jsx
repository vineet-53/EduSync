...
export default function EnrolledCourse({ course }) {
    return (
        <div className=' w-full flex items-center flex-col md:flex-row '>
            <div className='max-sm:w-11/12 h-full grid md:flex md:gap-2 grid-cols-1 max-sm:gap-2 bg-custom-secondary bg-opacity-30 px-3 py-3 rounded-md transition-all lg:w-3/5 lg:justify-between'>
                <div className=' flex flex-col md:flex-row gap-3'>
                    <img src={course?.thumbnailImage} alt={`${course?.courseName} thumbnail`} className='max-sm:mx-auto sm:max-w-32 rounded-md' />
